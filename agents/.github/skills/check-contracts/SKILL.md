...
accumulate — an outcome is always the most recent one for that test.
...
- **`unfulfilled test obligation`** — the clause names an uppercase `TODO` placeholder. Write the
  test and replace the placeholder. Expected during bootstrap; a real gap under `-Strict`.
...
Only an uppercase `TODO` marks an obligation, so a genuine test named `TodoItemsAreReturned` is
checked normally rather than being quietly exempted.

A name that survives only in a comment does not count as a living test — comments are stripped
before matching, and a clause is satisfied only by a real test-method declaration, so neither a
private helper nor a string literal can keep a deleted promise alive.
