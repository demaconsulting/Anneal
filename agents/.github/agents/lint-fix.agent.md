...
- **markdownlint MD013 (line length)**: Wrap long lines at natural break points,
  ...
  list if the data reads naturally that way, or drop a column. Do not get stuck
  trying to squeeze a wide pipe-table into 120 characters.
...
- **Contract test link failures**: A contract clause naming a test that does not
  ...
  `architecture-update` or `developer` agent — see the **check-contracts** skill.
