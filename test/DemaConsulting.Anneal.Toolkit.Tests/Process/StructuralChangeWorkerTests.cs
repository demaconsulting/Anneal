...
public class StructuralChangeWorkerTests
{
    ...
    {
        // Arrange: a tenet finding also routes through Developer, the same primitive a code finding uses, since
        // fixing a tenet violation is still a code-shaped fix
        var root = CreateTemporaryDirectory();
        try
        {
            var endpoint = new QueuedEndpoint(
            ...
            Assert.Multiple(
                () => Assert.Equal(OperationOutcome.Succeeded, result.Outcome),
                () => Assert.IsType<WorkerRunResult.Completed>(result.Finding),
                () => Assert.Contains(steps, line => line.Contains("\"Developer:tenet-repair\"")),
                () => Assert.DoesNotContain(steps, line => line.Contains("\"DocumentAuthor:repair\"")),
                () => Assert.DoesNotContain(steps, line => line.Contains("\"Developer:repair\"")),
                () => Assert.DoesNotContain(steps, line => line.Contains("\"Planner:replan\"")));
        }
    ...
    public async Task RunAsync_TenetRepairRequiredTwice_FailsNamingTheTenetConcernOnceItsBudgetIsSpent()
    {
        ...
        try
        {
            ...
            // Assert: failed, not rerouted and not re-planned - a spent budget is not a strategy finding, and the
    ...
    public async Task RunAsync_CodeAndTenetRepairsRequired_DispatchesBothAgainstIndependentBudgets()
    {
        ...
        try
        {
            ...
            Assert.Multiple(
                () => Assert.Equal(OperationOutcome.Succeeded, result.Outcome),
                () => Assert.IsType<WorkerRunResult.Completed>(result.Finding),
                () => Assert.Contains(steps, line => line.Contains("\"Developer:repair\"")),
                () => Assert.Contains(steps, line => line.Contains("\"Developer:tenet-repair\"")));
        }
        finally
        {
