...
prompt - it exists, is gated, and is already reachable through the ordinary `dispatch` → `route` path
whenever a work item classifies as Massive. Before designing a custom multi-step slicing plan for a
large piece of work, check whether describing the whole item to `dispatch` and letting it classify
Effort would get the same decomposition for free, with a stronger check than a hand-written one.
