...
- **`.anneal/`** — this folder, and the sole authoritative source of Anneal's own architecture,
  governance, and process-tracking documents (`config.json`: role-to-model mapping, the arguments a
  self-hosted run's contract check is invoked with), `skills/` where `file-skill` writes deliberately
  curated, committed lessons about this repository (see [Skills](../architecture/toolkit/skills.md)),
  `logs/records/` and `logs/transcripts/` (invocation and model-interaction logs, read by
  `dotnet anneal stats`), and the
  `governance/`, `profile/`, `work/`, and `architecture/` documentation folders this file belongs to.
