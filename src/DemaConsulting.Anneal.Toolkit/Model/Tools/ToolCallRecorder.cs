...
internal sealed class ToolCallRecorder
{
    ...
    ///     transcript. Refused and faulted calls are excluded: they did not produce evidence. Paths are
    ///     normalized on capture (backslashes unified to forward slashes, leading <c>./</c> and trailing
    ///     slashes stripped) so that equivalent spellings of the same path compare as equal. Comparison is
    ///     also case-insensitive because path capitalization is not a meaningful distinction on most platforms.
    /// </remarks>
